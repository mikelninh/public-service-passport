export const CITIZEN_CASE_HANDOFF_SCHEMA = 'psp.citizen-authority-handoff/1';
export const CITIZEN_CASE_HANDOFF_STORAGE_KEY = 'psp-v1-authority-handoff';
export const CITIZEN_CASE_HANDOFF_TTL_MS = 15 * 60 * 1000;
...
export function createCitizenCaseHandoff({ household, policyVersion = 'unknown', handoffId = 'local-preview', now = Date.now() } = {}) {
  const normalized = normalizeHousehold(household);
  const createdAt = Number(now);
  if (!Number.isFinite(createdAt) || createdAt < 0) throw new Error('now invalid');
  return {
    schema: CITIZEN_CASE_HANDOFF_SCHEMA,
    handoffId: String(handoffId).slice(0, 120),
    createdAt,
    expiresAt: createdAt + CITIZEN_CASE_HANDOFF_TTL_MS,
...
export function parseCitizenCaseHandoff(raw, { now = Date.now() } = {}) {
  let value;
  ...
  if (value.schema !== CITIZEN_CASE_HANDOFF_SCHEMA) throw new Error('handoff schema mismatch');
  ...
  if (!Number.isFinite(current)) throw new Error('now invalid');
  ...
  if (value.expiresAt - value.createdAt > CITIZEN_CASE_HANDOFF_TTL_MS) throw new Error('handoff TTL invalid');
...
export function toAuthorityPreviewCase(raw, options = {}) {
  const handoff = parseCitizenCaseHandoff(raw, options);
